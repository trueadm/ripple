...
With the regular destructuring, such as the one below, the `count` and `class`
properties would lose their reactivity:
...
>>>>>>> 4ae1575 (make raw files for docs 80ch max width)
:::
