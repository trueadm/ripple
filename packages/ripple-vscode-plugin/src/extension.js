 ...
 * 1. Starting a Volar-based language server (ripple-language-server) for Ripple syntax and semantics
 ...
 * --------------------------------------------------
 * ripple-language-server: A Language Server Protocol (LSP) server built on Volar that provides
 * language features for Ripple files including diagnostics, IntelliSense, go-to-definition, etc.
 * It uses typescript-plugin-ripple internally to transform Ripple syntax into TypeScript virtual
 * files for type checking and IntelliSense.
 *
 * typescript-plugin-ripple: A Volar-based TypeScript plugin that transforms .ripple files into
 * TypeScript virtual code. This plugin enables TypeScript's language service to understand Ripple
 * syntax. It's already loaded and used by ripple-language-server, so we don't need to configure
 * it separately.
 *
 ...
 * load typescript-plugin-ripple into its own tsserver instance. However:
 * 1. We already run ripple-language-server which uses typescript-plugin-ripple internally
 ...
 * -------------------------------------------------
 * The typescriptServerPlugins contribution point would load typescript-plugin-ripple into
 * the TypeScript extension's tsserver. However, we already run our own language server
 * (ripple-language-server) which uses typescript-plugin-ripple internally. Loading the
 * plugin twice would create conflicts and duplicate processing.
 *
 * By patching directly instead, we:
 * 1. Avoid double-loading typescript-plugin-ripple (it's already in our language server)
 ...
 * 4. Keep language intelligence in ripple-language-server while exposing TS UI features
 *
